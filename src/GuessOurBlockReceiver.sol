...
contract GuessOurBlockReceiver is IGuessOurBlock, OAppReceiver {
    uint32 public constant ONE_DAY_BLOCKS = 7200;
    ...
    mapping(address wallet => uint128) public failedNativeSend;
    ...
    function _guess(uint32 _blockNumber, uint32 _guessAmount) internal {
        ...
        if (block.number > _blockNumber || _blockNumber - block.number < ONE_DAY_BLOCKS) {
            revert BlockTooSoon();
        }
    ...
    function updateMinimumBlockAge(uint32 _minimumBlockAgeInSeconds) external onlyOwner {
        minimumBlockAge = _minimumBlockAgeInSeconds;
        emit MinimumBlockAgeUpdated(_minimumBlockAgeInSeconds);
    }
